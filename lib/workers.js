...
workers.rotateLogs = function () {
    // Lists non compressed  log files
...
workers.validateCheckData = function (originalCheckData) {
    ...
    if (originalCheckData.id && originalCheckData.protocol && originalCheckData.userPhone &&
        ...
        debug("Error in one of the checks is not properly formatted. Skipping it");
    }
}
...
workers.alertUserToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSMS(newCheckData.userPhone, msg, function (err) {
        if (!err) {
            ...
            debug('Error: could not send sms alert to user who had a state chage in their check');
        }
    })
}
